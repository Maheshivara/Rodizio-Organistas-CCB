...
class CadastroIgreja extends JInternalFrame {
	...
	private int incluir = 1, alterar = 2, salvar = 3, cancelarOuExcluir = 4, localizar = 5, carregar = 6, enable = 1,
			disable = 2;
	...
	public CadastroIgreja() {
		...
		cmbCidade.addFocusListener(new FocusAdapter() {
			...
			public void focusGained(FocusEvent e) {
				if (cmbUF.getSelectedIndex() < 1) {
					JOptionPane.showMessageDialog(rootPane, "Você Deve Selecionar um Estado Primeiro.");
					cmbUF.grabFocus();
				}
			}
			...
			public void focusLost(FocusEvent e) {
				if (cmbCidade.getSelectedIndex() < 1) {
					JOptionPane.showMessageDialog(rootPane, "Você Deve Selecionar uma Cidade.");
					cmbCidade.grabFocus();
				}
			}
		});
		 ...
		 * e Limpar a Tela.
		 */
		preparaArrayDeComponentes();// Insere os Componetes para Limpar ou travar a Edição
	...
	void insereBotoes() {
		...
		/**
		 * Botao Cancelar
		 * Ocupa a Mesma Posição do Botao Incluir
		 */
		...
		btnCarregar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (table.getSelectedRow() > -1) {
					...
					JOptionPane.showMessageDialog(rootPane, "Selecione uma linha e então Clique em Carregar.");
				}
			}
		});
		btnCarregar.setIcon(
		...
		/**
		 * Botao Incluir.
		 * Ocupa a Mesma Posiçao do Botao Cancelar
		 */
		...
		/**
		 * Botao Salvar.
		 * Ocupa a Mesma Posiçao do Botao Alterar
		 */
	...
	boolean validaFormulario() {
		...
		if (!ok) {
			JOptionPane.showMessageDialog(rootPane, "Ao menos um dia de Culto deve estar Selecionado.");
			return false;
		}
